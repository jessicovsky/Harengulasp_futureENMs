...
scenarios <- list.files('Figures/', pattern = 'Mess', full.names = TRUE)
sta       <- stack(scenarios)
title     <- c('Present', 'RCP 2.6', 'RCP 6.0', 'RCP 8.5')
...
wwf       <- readOGR(dsn = "Shape_files", layer = "Harengula_sp_M")
...
# Subset shapefile by Province

NBS    <- subset(wwf, PROVINCE =="North Brazil Shelf")
TSA    <- subset(wwf, PROVINCE =="Tropical Southwestern Atlantic")
WTSA   <- subset(wwf, PROVINCE =="Warm Temperate Southwestern Atlantic")

plot(NBS, col = cNBS, xlim = c(-60, -20), 
     ylim = c(-45, 2))
plot(TSA, col = cTSA, add = TRUE)
plot(WTSA, col = 'chartreuse4', add = TRUE)

plot(map, xlim = c(-60, -20), 
     ylim = c(-45, 2), col = 'lightgray',
     lwd = 0.001, xaxt = 'n', yaxt = 'n',
     main = title[i], font = 2, add= TRUE)


# Training testing

occ <- read.csv('Datasets/Harengula_sp_original.csv')
unique(occ$Database)

GBIF      <- occ[occ$Database == 'GBIF',]
Fish2Net  <- occ[occ$Database == 'Fish2Net',]
SpLink    <- occ[occ$Database == 'SpLink',]
Ronaldo   <- occ[occ$Database == 'Ronaldo',]
Gasparini <- occ[occ$Database == 'Gasparini',]
Sergio    <- occ[occ$Database == 'Sergio',]
Thais     <- occ[occ$Database == 'Thais',]
Aninha    <- occ[occ$Database == 'Aninha',]


Local_colection <- rbind(Ronaldo, Gasparini, Sergio, Thais, Aninha)

points(GBIF[,2:3], pch = 21, bg = 'white', col = 'black')
points(test[,2:3], pch = 21, bg = 'grey', col = 
         'black')


legend("bottomright", 
       c("North Brazil Shelf",
         "Tropical Southwestern Atlantic",
         "Warm Temperate Southwestern Atlantic"), fill= c('darkolivegreen3', 
                                                          'aquamarine4', 'chartreuse4'),
       cex = 0.7, bg = 'white')
box()
degAxis(1)
degAxis(2, las = 2)

compassRose(-75, 4, rot = 0, cex = 0.95) #Agregar Rosa de Vientos

scalebar(1500, xy = c(-80, -40), type = "bar", divs = 4, below = "Kilometers", col = 'black',
         lonlat = TRUE, cex = 0.9) 
